    ...
    internal static class Sbrf31
    {
        ...
        public static InfoResult Void(string RRN)
        {
            try
            {
                ...
                return result.Equals(SberbankReturnCodes.OK) ? ReturnOK(server) : ReturnError(result, "Ошибка при отмене операции");
            }
            catch (Exception ex)
            {
                return ReturnException(ex);
        ...
        public static InfoResult Pay(decimal amount)
        {
            try
            {
               ...
               // _server = null;
                ...
                infoResult.SymbolE = GetSymbolE(infoResult.Cheque);
                return infoResult;
            }
            catch (Exception ex)
            {
        ...
        private static string GetSymbolE(string sbrfCheque)
        {
            string[] list = sbrfCheque.Replace("\r\n", " ").Split(' ');
            for (int i = 0; i < list.Length; i++)
            {
